...
) -> Result<(StatusCode, Json<AudioFile>), (StatusCode, Json<serde_json::Value>)> {
    loop {
        ...
        if field.name() == Some("file") {
            ...
            return match AudioRepository::create(
                &state.db,
                &file_id,
                &auth_user.id,
                &new_filename,
                ...
                Ok(audio_file) => Ok((StatusCode::CREATED, Json(audio_file))),
                Err(e) => {
                    tokio::fs::remove_file(&file_path).await.ok();
                    Err((
                        StatusCode::INTERNAL_SERVER_ERROR,
